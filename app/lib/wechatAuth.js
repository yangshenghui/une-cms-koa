...
class WechatAuth {
  ...
  get instance () {
    return new WechatAuth(config.getItem('wx.appid', ''), config.getItem('wx.appsecret', ''))
  }
  ...
  getAuthorizationUrl (scope) {
    const redirect_url = encodeURIComponent(config.getItem('wx.authorize_redirect_url', ''))
